...
import merkleTree  from '../../public/blog-assets/merkle-tree.png'
import { OptimizedImage } from '../../themes/MDXStyles.tsx';

<OptimizedImage alt='test'  src={merkleTree}
title='The value of a node depends on the value of the leaves.'
/>
...
import stateMod from '../../public/blog-assets/tx-state-mod.png'

<OptimizedImage src={stateMod} alt='Transactions as state modifications'
title='Transactions as state modifications'
/>
...
import accountStruct from '../../public/blog-assets/eth-account.png'
...
<OptimizedImage src={accountStruct}
alt="Etherem account structure"
title="Structure of an account"/>

# State storage

So where is all this data stored? The blockchain implicitly stores state in the block headers, along with other data.

import stateStorage from '../../public/blog-assets/state-storage.png'

<OptimizedImage src={stateStorage}
alt='State storage'
title='Each block header contains the hash of the root node of the "World State Trie"'
/>
...
export const meta = {
    ...
    long_desc: "Bitcoin is often described as a distributed ledger which facilitates a decentralized currency using cryptographic tools. A cryptocurrency behaves like a “normal” currency because of the rules governing the modification of the ledger. For example, a Bitcoin address cannot"
}
